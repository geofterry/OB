...
namespace Offline_Backend_PT_Aksi_Visitama_Project_Application
{
    public partial class UploadGeneration : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog1.InitialDirectory = "C:\\Users\\Lenovo Admin\\Desktop\\PT AV\\Tada Gen";
                ...
                if (temptadano != 0)
                {
                    ...
                    for (int i = 0; i < tempGenID.Count; i++) // Loop through List with for
                    {
                        if (i == 0)
                        {
                            ...
                            con.executeUpdate("insert into generation values('" + tempGenID[i] + "','" + dataGridView1.Rows[i].Cells[4].Value.ToString() + "' , '" + tempJumlahList[i] + "', '" + (dataGridView1.Rows[i].Cells[0].Value.ToString().Substring(6, 4) + "-" + dataGridView1.Rows[i].Cells[0].Value.ToString().Substring(3, 2) + "-" + dataGridView1.Rows[i].Cells[0].Value.ToString().Substring(0, 2)) + "', '" + dataGridView1.Rows[i].Cells[6].Value.ToString() + "','" + dataGridView1.Rows[i].Cells[5].Value.ToString() + "','" + MDIform.username + "')");
                        }
